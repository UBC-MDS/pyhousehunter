...
def test_cleaner_scraped_df_is_dataframe():
    """
    Test to confirm that ValueError is raised when the required scraped_df input is not a dataframe
    """
...
def test_cleaner_scraped_df_not_missing_price():
    ...
    """
    with raises(ValueError):
        cleaner.data_cleaner(pd.DataFrame([[1, 2]] , columns = ['house_type', 'listing_url']))
